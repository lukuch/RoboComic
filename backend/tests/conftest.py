...
def mock_llm(monkeypatch):
    monkeypatch.setattr(
        "services.llm_utils.generate_topic_context_llm",
        lambda *args, **kwargs: "This is a mock topic context."
    )
...
def mock_tts(monkeypatch):
    monkeypatch.setattr(
        "tts.eleven_tts_service.ElevenTTSService.speak",
        lambda self, *args, **kwargs: b"FAKE_WAV_DATA"
    )
    monkeypatch.setattr(
        "tts.bark_tts_service.BarkTTSService.speak",
        lambda self, *args, **kwargs: (b"FAKE_WAV_DATA", 22050)
    )
    yield 
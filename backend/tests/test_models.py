...
class TestTTSRequest:
    ...
    def test_empty_text(self):
        with pytest.raises(ValidationError):
            TTSRequest(text="", lang="en")


class TestGenerateShowResponse:
    def test_valid_response(self):
        chat_history = [ChatMessage(role="user", content="Test joke 1"), ChatMessage(role="assistant", content="Test joke 2")]
        response = GenerateShowResponse(history=chat_history, success=True, message=None)
...
class TestPersonasResponse:
    def test_valid_response(self):
        personas = {"relatable": {"id": "relatable", "name": "Relatable", "description": "Relatable style"}}
...
class TestHealthResponse:
    def test_valid_response(self):
        response = HealthResponse(status="healthy", version="1.0.0", timestamp="2024-01-01T12:00:00Z")
...
class TestTemperaturePresetConfig:
    def test_valid_preset(self):
        preset = TemperaturePresetConfig(name="Creative", temperature=0.8)
...
class TestChatMessage:
    def test_valid_message(self):
        message = ChatMessage(role="user", content="Hello, this is a test message.")
